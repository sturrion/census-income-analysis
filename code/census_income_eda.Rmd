...
output:
  html_document:
    ...
    toc_depth: 4
    includes:
      in_header: sturrion_header.html
  pdf_document:
    highlight: zenburn
    toc: yes
...
adult.data <- read.csv('../data/adult.data', stringsAsFactors = FALSE, header = FALSE, col.names = column.names, strip.white = TRUE)
...
```{r}
adult.data$age.segment <- cut(adult.data$age, breaks = c(0,17,27,37,47,57,67,77,87,100), 
                              labels = c("< 18", "18-27", "28-37", "38-47","48-57","58-67","68-77","78-87", "> 87"))
...
```{r}
adult.data$capital.scaled <- scale(adult.data$capital, center = TRUE, scale = TRUE)
summary(adult.data$capital.scaled)
```

And we discretize it.  
```{r}
adult.data$capital.segment <- cut(adult.data$capital.scaled, breaks = c(-1.0, 0.0, 1.0, 14.0), 
...
```{r}
adult.data$hours.per.week.segment <- cut(adult.data$hours.per.week, breaks = c(0,9,19,29,39,49,59,69,79,89,100), labels = c( "1-9", "10-19", "20-29", "30-39","40-49","50-59","60-69","70-79","80-89","90-99"))
